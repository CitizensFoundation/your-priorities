...
(function($) {
	...
	**********************************************************************************/
	$.fn.NobleCount.settings = {
		...
		block_negative: true,  // if true, then all attempts are made to block entering
								//		more than max_characters
		cloak: false,			// if true, then no visual updates of characters 
								// 		remaining (c_obj) occur
		default_txt: '',        // Show max numbers if default text same as current text
	...
	**********************************************************************************/
	...
	function event_internals(t_obj, char_area, c_settings, max_char, init_disp) {
		...
		if (c_settings.cloak) {
            ...
            //alert($(t_obj).val());
            //alert(c_settings.default_txt);
            if ($(t_obj).val()==c_settings.default_txt) {
                char_area.text(c_settings.max_chars);
            } else {
