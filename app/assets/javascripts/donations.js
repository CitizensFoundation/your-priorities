function handlePaymillResponse(error, result) {
    if (error) {
        ...
        $('#donation_paymill_card_token').val(result.token);
        //alert(result.token);
        //alert("AFTER NOT ERROR");
        return $('#new_donation')[0].submit();
    };
}
...
function processCard() {
  var card;
    card = {
      ...
      amount_int: Math.round(parseFloat($('#donation_amount').val()) * 100),
...
function processErrorCodes(error_code) {
    error_response = 'Unkown error with card payment: '+error_code;
    switch (error_code) {
