class RssController < ApplicationController
  ...
  def your_notifications
    @page_title = tr("{user_name} notifications at {instance_name}", "controller/rss", :user_name => @user.login.possessive, :instance_name => current_instance.name)
  ...
  def your_comments
    @page_title = tr("Latest comments on {user_name} discussions", "controller/rss", :user_name => @user.login.possessive, :instance_name => current_instance.name)
  ...
  def your_ideas_created_activities
    @page_title = tr("Everything happening on ideas you created", "controller/rss", :instance_name => current_instance.name)
    @activities = nil
    created_idea_ids = @user.created_ideas.collect{|p|p.id}
    if created_idea_ids.any?
      @activities = Activity.active.by_recently_created.paginate :conditions => ["idea_id in (?)",created_idea_ids], :page => params[:page], :per_page => params[:per_page]
    end
    respond_to do |format|
