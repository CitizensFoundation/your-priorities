...
class ApplicationController < ActionController::Base
  ...
  def check_geoblocking
    Rails.logger.info("action_log - #{session[:session_id]} - #{request.remote_ip} - #{controller_name}/#{action_name} - #{@country_code} - #{current_locale} - #{current_sub_instance.short_name} - #{current_user ? current_user.login : "Anonymous"} - #{current_user ? current_user.id : "-1"} - #{request.user_agent}")
